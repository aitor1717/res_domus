...
def _urgency_color(pct: int) -> str:
    if pct >= 85: return "#F92672"
    if pct >= 70: return "#FD971F"
    if pct >= 55: return "#E6DB74"
    if pct >= 45: return "#75715E"
    if pct >= 30: return "#66D9E8"
    return "#454158"
...
def recent_orders():
    ...
    for row in rows:
        ...
        if "pedidosya" in sf.lower() or "pedidos" in row["source"].lower():
            row["entry_type"] = "app"
        elif sf.endswith(".csv") and "img" not in sf:
            row["entry_type"] = "texto"
        else:
            row["entry_type"] = "foto"
