...
from parser.prompts import build_parser_prompt
...
def _parse_thread(sid: str, app) -> None:
    with app.app_context():
        ...
        try:
            ...
            prompt = build_parser_prompt(canonical_items, confirmed_date_str)
            _emit(sq, "progress", {"message": "Parsing receipt…"})

            client = anthropic.Anthropic(api_key=current_app.config["ANTHROPIC_API_KEY"])
            items = parse_group(client, sess["images"], prompt)
...
def retry_parse():
    ...
    base_prompt = build_parser_prompt(canonical_items, order_date)
    prompt = base_prompt + (f"\n\nAdditional instruction: {note}" if note else "")
